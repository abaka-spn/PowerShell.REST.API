    ...
    static class Program
	{
		...
		static int Main(string[] commandLineArguments)
		{
			...
			catch (Exception eUnexpected)
			{
				...
				Console.ReadLine();
				PowerShellRestApiEvents.Raise.UnhandledException(eUnexpected.Message);
				return 1;
			}
		}
		...
		private static int RunAsConsole()
		{
			try
			{
				...
				PowerShellRestApiEvents.Raise.ApiStart();
			...
			catch (CertificateNotFoundException certMissing)
			{
				...
				PowerShellRestApiEvents.Raise.ConfigurationError("Cannot find certificate to authenticate requests - " + certMissing.Message);
				Console.ReadLine();
				return -1;
			}
			catch (System.Reflection.TargetInvocationException invocationException)
			{
					...
					invocationException.InnerException.GetType() == typeof(CertificateNotFoundException))
				{
					...
					PowerShellRestApiEvents.Raise.ConfigurationError("Cannot find certificate to authenticate requests - " + invocationException.InnerException.Message);
					Console.ReadLine();
					return -1;
				}
			}
			catch (Exception ex)
			{
				...
				PowerShellRestApiEvents.Raise.UnhandledException(ex.Message);
				Console.ReadLine();
				return -1;
			}
			Console.WriteLine("To get server status, make a request to : {0}{1}", Startup.BaseUrl, Constants.StatusUrlPath.Substring(1));
            Console.ReadLine();

			// Raise a stop service
			PowerShellRestApiEvents.Raise.ApiStop();
		...
		static int RunAsService()
		{
			try
			{
				...
				ServiceBase.Run(new PowerShellRestApiService());
			}
			catch (CertificateNotFoundException certMissing)
			{
				Console.WriteLine("Cannot find certificate to authenticate requests - " + certMissing.Message);
			...
			return
				new TransactedInstaller
				{
					...
					Installers =
					{
						...
						{
							Context = installContext,
							ServiceName = "PowerShellRestApi",
							DisplayName = "PowerShell REST API Service",
